...
declare
  ...
  v_display_name_changed boolean := true;
begin
  ...
  if found then
    v_display_name_changed := v_existing.display_name <> v_display_name;
  end if;

  if found
     and not v_display_name_changed
     and v_existing.avatar_id = v_avatar_id
     and coalesce(v_existing.avatar_url, '') = coalesce(v_avatar_url, '')
  ...
  if found and v_display_name_changed and v_existing.last_changed_at > now() - interval '1 month' then
  ...
  on conflict (user_id) do update
     set display_name = excluded.display_name,
         ...
         last_changed_at = case
           when v_display_name_changed then excluded.last_changed_at
