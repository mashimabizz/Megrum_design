        ...
        ProposalCardSection(title: "待ち合わせ候補") {
            VStack(alignment: .leading, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    ProposalMeetupPlaceSuggestionChips(suggestions: placeSuggestions) { suggestion in
                        placeName = suggestion
                    }
                }

                ProposalMeetupLocationStatusMessage(
                    isRequestingLocation: isRequestingLocation,
                    message: locationErrorMessage ?? "現在地が使える場合は緯度経度を自動入力します。"
                )
            }
        }
        .onAppear {
