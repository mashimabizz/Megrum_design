...
- `Sources/MegrumData/SupabaseGoodsInventoryClient.swift` contains the request layer for reading `goods_types_master`, creating `goods_inventory` rows for Inventory/Wish, searching tradeable goods, reading another user's public trade goods, and archiving or deleting owned goods rows.
...
- `Sources/MegrumData/SupabaseProposalClient.swift` contains the request layer for loading, creating, accepting, and rejecting `proposals`.
...
- `Sources/MegrumData/SupabaseScheduleClient.swift` contains the request layer for loading personal `schedules` for trade calendar overlays.
...
`GoodsGrid` is shared by Home, Search, Inventory, and Wish surfaces. It now opens a native detail sheet on tap and uses SwiftUI's native context menu for long-press actions, keeping the action surface close to iOS Home Screen quick actions while preserving standard accessibility behavior. Search reads tradeable goods through `MegrumAppState`, groups results into match buckets, and exposes group/goods type filter chips from the same master data as Inventory and Wish. From a search result, the exchange-list action can open a native proposal creation sheet that selects one of the viewer's inventory items, chooses an exchange method and condition tags, then creates a `proposals` row through `SupabaseProposalClient`. Inventory and Wish collection screens also expose a native 3/4/5-column toggle, group/goods type filter chips, and a left-side add button. The add button opens a native sheet that loads groups and goods types, then creates a `goods_inventory` row through `MegrumAppState`. Wish now has a native segmented switch for "Wish" and "個別募集"; the latter reads `listings` with their `listing_wish_options`, shows compact native cards, and creates a new individual listing from the viewer's inventory and wishes through `SupabaseListingClient`. A viewer-owned Wish tile can also open the same individual listing sheet from the native long-press menu, with that Wish preselected as the requested item. On the viewer's own Inventory/Wish tiles, the long-press menu can archive the row with `status='archived'` or delete the owned row after a native confirmation dialog. On another user's goods tile, the long-press report action opens a native report sheet and files a `goods_reports` row through `SupabaseGoodsReportClient`.

When a grid tile belongs to another user, the primary tap opens a native public profile sheet. `SupabaseUserProfileClient` loads the profile summary through `get_public_user_profile_for_viewer` and the rating list through `list_user_evaluations_for_profile`, so the app keeps direct `user_evaluations` RLS narrow while still showing profile-safe rating data. The public profile also loads that user's public trade goods and active individual listings, displays them with a native segmented control, and can open the existing proposal creation sheet from either a goods tile or a listing card. Listing-origin proposals now pass `listing_id` through the Swift proposal payload.

Image-based member suggestion is split into three replaceable layers. `VisionFaceDetectionService` uses Apple's Vision framework only to detect face rectangles in uploaded images; it does not use Face ID or biometric identity APIs. `FaceEmbeddingService` is a protocol boundary for a production Core ML model or a trusted backend embedding endpoint. The default app implementation intentionally returns `embeddingProviderUnavailable` so previews and tests do not pretend that a face-recognition model is bundled. `DefaultFaceTaggingService` then combines detections, embeddings, and `MemberFaceProfile` rows with cosine-similarity thresholds (`auto_matched >= 0.90`, `needs_review >= 0.70`, otherwise `unknown`; low-quality detections stay `low_quality`). `FaceTaggingReviewSheet` is the reusable confirmation UI: it shows the uploaded image without drawing detection boxes, shows candidate members and confidence, and allows manual correction. Goods photo selection now runs the detection service when a chosen or cropped image has faces and applies the confirmed member to the current draft/photo metadata. The Supabase foundation lives in `member_face_profiles`, `face_uploaded_images`, `detected_faces`, `face_match_candidates`, and `face_match_corrections`; member profile writes are reserved for service-role/admin flows with recorded consent. Goods registration first scopes member suggestion to the selected oshi context: `group` and `work` entries load only their own `characters_master` rows and matching `member_face_profiles`, while `solo` entries hide member selection and skip member inference.

Member suggestion can now route uploaded images by image type before recognition. `ImageTypeClassifier` classifies an upload as `real_photo`, `anime`, `illustration`, `manga`, or `unknown`; the default classifier only treats an image as `real_photo` when Vision can conservatively find a real face, and otherwise returns `unknown` rather than pretending high precision. `UnifiedMemberTaggingService` sends `real_photo` images through the existing `DefaultFaceTaggingService`, while `anime`, `illustration`, and `manga` are sent to `AnimeRecognitionService`. The current app uses `UnavailableAnimeRecognitionService`, which is intentionally safe: when no anime/illustration model or backend API is configured, it returns `unknown` with no crash and no fake confidence. A production implementation can replace that service with an anime face detector, character classifier, CLIP-like image embedding endpoint, or Core ML model.

Real-photo thresholds and anime/illustration thresholds are configured separately through `MemberTaggingThresholds`: real photos keep `auto_matched >= 0.90` and `needs_review >= 0.70`, while anime/illustration/manga require `auto_matched >= 0.95` and `needs_review >= 0.75`. These image-type labels are internal routing data and should not be shown as prominent user-facing choices in the confirmation sheet. Results carry a unified member-tagging shape: `image_type`, `subject_type`, `recognition_method`, `quality_status`, `model_version`, and `profile_type` are stored alongside the existing detected-face rows. `member_face_profiles.profile_type` separates `real_face`, `anime_face`, `anime_character`, and `illustration_embedding` learning data so future models do not mix incompatible embeddings. Manual corrections also persist the image/subject/recognition/profile fields, and the training-data flag is enabled by default without a user-facing off switch.

`TradesScreen` splits proposals into "打診中" and "進行中" with a floating native segmented control above the tab bar. Horizontal swipes switch the stage, and tapping a trade card opens a native detail sheet. The detail sheet now loads trade chat messages through `MegrumAppState`, shows native text/photo message bubbles, can open shared photos full-screen, and can send text messages through `SupabaseMessageClient`. Incoming pending proposals can be accepted or rejected from the native detail sheet; acceptance updates `agreed_by_sender` / `agreed_by_receiver` and moves the proposal to `agreement_one_side` or `agreed`, while rejection moves it to `rejected`. If the proposal allows both handoff and mail, the acceptance panel uses a native segmented picker to choose the final `exchange_method` before updating the proposal. If the user wants to change conditions, the response panel and the trade chat input area can open the same native counter-proposal sheet. The sheet copies the original goods from the viewer's perspective, adjusts exchange method, condition tags, and message, then creates a new `negotiating` proposal with `agreed_by_sender=true` instead of mutating the original. The trade chat input area also exposes a native schedule sheet that loads both participants' `schedules`, shows a 5-day week view and a month view, and displays schedule titles plus text place names with viewer/partner color coding. For agreed trades, the native detail sheet can capture an evidence photo with the iOS camera or choose one through PhotosPicker, upload it to `chat-photos`, mirror it to `proposal_evidence_photos` and `proposals.evidence_photo_url`, open the evidence photo full-screen, approve evidence from each side, complete the proposal when both sides approve, and submit a `user_evaluations` rating. The detail sheet also exposes a native report sheet that files the existing `disputes` record and leaves a system message in the trade chat.
