...
> **目的**：弁護士納品の規約原典（2024-07-18）と現状の Megrum 設計（2026-05-01）の関係を整理。
> **MVP 段階では docx 改訂不要・現行規約をそのまま運用** という方針を採用（iter47）。

最終更新: 2026-05-29（iter168.71 方針差分追記）
ステータス: 現地交換前提の法務整理に、郵送交換再追加の差分メモを追記

---

## 2026-05-29 追記：郵送交換を再びスコープに入れる判断

- プロダクト方針として、現地交換に加えて **郵送交換** を初回リリース候補へ戻す判断が入った。
- このドキュメントの本文は、**iter46-47 時点の「MVP は現地交換のみ」前提** で書かれているため、現状のままでは整合しない。
- 特に次の論点は、公開前に法務・利用規約・プライバシーの再確認が必要：
  - 住所を当事者間へ開示する条項
  - 本人確認を入れない運用での注意書き
  - 発送・受領・未着トラブル時の責任分界
  - 通報・アカウント制限・補償しない範囲の明記
- したがって、**「郵送交換なしなら現行規約で提出可」だった前提は、いったん保留** とする。
...
| 13.5 | めぐりPlus月額利用料 | 規定なし | ¥1,000/月 | **特商法・有料サービス条項に追加** |
