...
> iter47 では「docx 改訂不要・現行規約をそのまま運用」方針だった。2026-05-31時点の初回提出は、現地交換MVPへ再固定し、公開前ドラフトはその前提で管理する。

最終更新: 2026-05-31（iter398 初回提出スコープを現地交換MVPへ再固定）
...
- 現行仕様（Swift Native iOS主線、現地交換、めぐり、グルーム、スポット掲示板、Premium/めぐりPlus/ブースト、APNs通知等）を前提に、次の公開前ドラフトを作成した。
...
- 現在地共有・服装写真は任意共有、取引相手限定表示、原則取引終了後30日を目安に削除又は非表示化する方針で文面化した。ただし、異議申し立て・通報・法令対応時は必要な期間保存できる。
