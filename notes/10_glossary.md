...
| **まとめて登録** | trading card bulk crop, AI bulk registration, 旧称:トレカAI一括登録 | トレカ種別の譲登録で、複数枚を写した1枚の写真から端末内Vision/Core Imageでカード枠を検出し、トリミング画面に初期枠として自動配置する機能。UIラベルを「まとめて登録」へ変更（iter1226.431） | iter168.49, iter1226.431 |
| **トリミング（グッズ登録）** | goods photo crop | グッズ登録の写真追加直後に必ず通る切り抜き画面。初期枠は写真全体で、枠のドラッグ移動・四隅リサイズ・枠追加ができる。まとめて登録との違いは「AIが枠を自動配置するか」だけ | iter1226.431 |
| **生写真** | なまがみ | 印刷写真 | — |
