...
| **推しでマッチ** | 旧: メンバーでマッチ | ホーム下段のマッチセクション。推し単位の塊で候補を出す。行は画像右・テキスト左のミラー配置 | iter1226.272 |
| **強タグ** | strong tag | ホーム候補行に1個だけ出す成立しやすさの分類タグ。語彙は「ぴったり／指名あり／会えそう／郵送OK／wish一致／要相談」の6種。旧3条件タグ（グッズ◎/交換◎/支払?）の後継 | iter1226.272 |
| **結論一文** | summary line | 候補行の強タグ下に出す1〜2行の説明文。「節A（グッズ軸）・節B（交換軸）」を合成し、県名（短縮形）・日付・会場などの具体値を出す。要相談時は最も軽い相談ポイント1個＋相手の実値。支払unknownは文に出さない | iter1226.272 |
