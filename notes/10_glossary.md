...
最終更新: 2026-07-06
ステータス: Draft v4.13（iter1226.345 ガイドツアーv3.1＝章×ビート・実演デモを反映）
...
| **ガイドツアー** | guide tour, オンボーディングツアー | 初期設定完了直後に初回のみ自動起動する使い方案内。10章・約50ビートをタップで進む（ようこそ→ホーム3セクション→タブ帯予告→マイグッズ登録デモ→ほしいもの→個別募集作成デモ→打診デモ→やりとり→めぐり→完了）。マイグッズ/個別募集/打診は指アイコンが実UI部品を操作してみせるデモステージ（分離プレビューAppState・実データ不変）。スポットライトは対象だけ明るく切り抜き、吹き出しは章内カウンタ・「この章をとばす」・次へを常設。めぐりはサンプルピン表示＋指タップで実物の作成コールアウトを出す実演。既読は端末ローカル（per-user UserDefaults）に保存し二度と自動表示しない。iOS標準TipKitでは実現不能なためカスタムオーバーレイ | iter1226.336, iter1226.337, iter1226.345 |
