...
最終更新: 2026-07-06
ステータス: Draft v4.12（iter1226.337 ガイドツアーv2＝10ステップ・隣接吹き出しを反映）
...
| **ガイドツアー** | guide tour, オンボーディングツアー | 初期設定完了直後に初回のみ自動起動する使い方案内。タップで進む10ステップ（ようこそ→ホーム3セクションを1つずつハイライト→マイグッズ→ほしいもの→個別募集→やりとり→めぐり→完了）。スポットライトは対象だけ明るく切り抜き、吹き出しは対象に隣接配置（n/10カウンタ・次へ・スキップ常設）。めぐりはdimなしの下部バナーでマップ全面を見せる。既読は端末ローカル（per-user UserDefaults）に保存し二度と自動表示しない。iOS標準TipKitでは実現不能なためカスタムオーバーレイ | iter1226.336, iter1226.337 |
