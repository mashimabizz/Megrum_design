...
ステータス: Draft v0.2（公開後・レビュー運用前、App Store評価・公開レビュー返信の個人情報境界）
...
Apple公式ヘルプでは、低評価、技術的な問題、不具合、改善要望、誤解があるレビューを優先して返信する運用が示されている。また、返信には個人情報、マーケティング表現、スパム、攻撃的又は不適切な表現を含めず、問題のあるレビューは公開返信で反論するよりもAppleへconcern reportを検討する。
...
| RPLY-007 | 個人情報、取引情報、認証情報、金融機関情報、証跡URL、スクリーンショットを引用又は再掲していない | TODO |
| RPLY-008 | マーケティング、値引き、レビュー変更依頼、レビュー対価、スパム、攻撃的表現がない | TODO |
| RPLY-009 | レビュー本文に個人情報又は秘密情報がある場合、返信より先にconcern report又はサポート誘導を検討した | TODO |

No-Go:
- レビュー本文に含まれる個人情報を引用する。
- レビュー本文に含まれるメールアドレス、電話番号、住所、取引ID、注文番号、認証コード、証跡URL、スクリーンショット、金融機関情報、本人確認書類を公開返信で拾う。
- 相手方ユーザーの処分、内部審査、内部ログの内容を書く。
- 「必ず直します」「補償します」「返金します」と断定する。
- 事故疑いを確認前に否定する。
- App Store公開返信だけで個別サポートを完了扱いにする。
- 評価変更、削除、星の増加、レビュー投稿を依頼、誘導、対価化する。
- 公開返信でキャンペーン、値引き、外部SNS誘導、広告宣伝を行う。
- 感情的、皮肉、反論調、攻撃的又はユーザーを責める表現にする。
...
- サポート窓口への誘導。
- 公開してよい範囲の一般的な確認状況。
- パスワード、認証コード、金融機関情報等を送らない注意喚起。

公開返信でしないこと:
- 個別ユーザーの本人性、アカウント状態、課金状態、取引状態、通報状況、相手会員への措置を示す。
- レビュー本文の個人情報、秘密情報、スクリーンショット内容を復唱する。
- 事故、障害、返金、補償、法的責任、相手会員への処分を確定させる。
- サポート受付番号以外の内部ID、ログ、管理画面情報、審査情報を出す。

レビュー本文に公開に適しない情報が含まれる場合:
1. 返信前にスクリーンショット又はメモをレダクションして証跡化する。
2. Appleへconcern reportを検討する。
3. 公開返信は個人情報を引用せず、`support@megrum.jp` へ誘導する。
4. P0事故疑いなら `notes/49` へ切り替える。
