...
1. **指名（ほしいもの1件）** → 選択肢の `wishIds` は**募集オーナー自身のほしいもの**を指す。そのほしいもののグループ・メンバー・種別・シリーズと自分のグッズを軸判定。
2. **条件指定** → グループ・種別＋メンバー（指定/除外）・シリーズ・数量で軸判定。
3. **現金** → グッズ合致の対象外（定価行で扱う）。

> ⚠️ iter1226.368：以前あった「選択肢が**自分（viewer）の在庫IDを直接指名**していれば確定」という **別ユーザー間ID一致マッチは撤去**。`wishIds` はオーナー自身のほしいもの行であり、他ユーザーの在庫IDと一致することは無い（経路として存在しない）。合致は必ず**属性（グループ・メンバー・種別・シリーズ）**で判定する。
