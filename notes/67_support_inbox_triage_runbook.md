...
| 位置情報・近距離公開 | 現在地共有の誤送信、近距離投稿から自宅/学校/勤務先/宿泊先/座席/未成年者の居場所が推測される、1km/3km表示への誤解、現地交換モードOFF後の保持相談 | P0/P1 | 対象投稿/取引/メッセージを特定し、危険時は警察・会場・施設管理者等を案内。必要に応じて非表示/制限/Incidentへ。削除、相手端末からの回収、匿名化、安全確保、責任認定を保証しない | `notes/25`, `notes/52`, `notes/54`, `notes/legal/01`, `notes/legal/02` |
...
- 現在地共有、近距離投稿、現地交換モード、待ち合わせ候補について、相手端末の保存、スクリーンショット、通知、キャッシュ、通報証跡、監査ログからの完全回収又は即時完全削除を約束する。
