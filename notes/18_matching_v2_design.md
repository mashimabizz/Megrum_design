...
## 2026-06-13 現行ホーム判定仕様（最新）

この節は、2026-06-13 のオーナー確認で確定したホーム判定仕様である。過去節にある「完全マッチ / 私を求めてる人 / 私が求めてる人 / 探索」型の棚分けは履歴として残すが、Swift Native の現行ホームでは採用しない。

### ホーム表示棚

ホームは、まず `グッズ条件`、`交換条件`、`支払い条件` をカード単位の最小判定として持つ。そのうえで、表示棚は現行デザインどおり次の3つにする。

| 表示棚 | 出す条件 | 備考 |
|---|---|---|
| ユーザー×タグでマッチ | 自分の Wish が相手の譲候補に一致し、かつグッズタグが1つ以上一致 | タグ一致は棚分け・並び順に使う。`グッズ条件` の◎/○/△には含めない。 |
| ユーザーでマッチ | 自分の Wish が相手の譲候補に一致し、タグ一致はない | メンバー/キャラクター単位の一致を見せる。 |
| 譲るものから見る | 自分の譲在庫を起点に、それを求めている相手を出す | 自分の Wish には一致しないが、相手の個別募集には一致するケースをここで拾う。 |

`譲るものから見る` は、自分の在庫起点で「それを求めている相手」を探す一覧である。相手の個別募集に一致する件数や強さが多い相手を上位に出す。

### グッズ条件

`グッズ条件` は、相手が自分の譲候補を求めている根拠だけで判定する。タグ一致は含めない。

| 判定 | 定義 |
|---|---|
| グッズ条件◎ | 相手の個別募集に、自分の譲候補が一致している。 |
| グッズ条件○ | 相手の Wish に、自分の譲候補が1つ以上一致している。Wish が双方向に一致していても◎にはしない。 |
| グッズ条件△ | 相手の Wish / 個別募集の中に、自分が譲候補として持っているグッズが1つもない。 |

`グッズ条件△ × 交換条件◎` もホームに出す。これは「交換方法や場所は合うが、相手が自分の譲候補を明確には求めていない」候補として扱う。

### 交換条件

`交換条件` は、交換方法と都道府県で判定する。日程は◎/○/△の安定条件には含めない。

| 判定 | 定義 |
|---|---|
| 交換条件◎ | 発送OK（`mail`）同士、または現地交換OK同士かつ都道府県が一致している。 |
| 交換条件○ | 現地交換OK同士だが、都道府県が一致しない。 |
| 交換条件△ | 共通する交換方法がない、または交換方法が未設定で判定不能。 |

発送（`mail`）のみの交換では、都道府県一致を条件に含めない。

### 支払い条件

`支払い条件` は、定価交換や差額相談など、金銭の受け渡しが関係する可能性がある候補で表示する。Megrum がアプリ内決済を処理する意味ではなく、ユーザー同士が対応できる支払い方法の自己申告条件として扱う。

| 判定 | 定義 |
|---|---|
| 支払い条件○ | 自分と相手の支払い条件のうち、判定対象の方法が1つ以上一致している。 |
| 支払い条件△ | 判定対象の方法が1つも一致しない、または片方が未設定。 |

支払い条件として保存できる候補は、`銀行振込` / `PayPay` / `現金交換` / `その他` とする。このうち、○/△判定に使うのは `銀行振込` / `PayPay` / `現金交換` の3つだけ。`その他` は保存・表示用で、ホームの支払い条件判定には使わない。

`口座` は独立した支払い条件として扱わない。口座番号などの詳細情報は、ホーム判定用の支払い条件には保存しない。必要になった場合も、合意後の取引内で扱う。

### タグ判定と並び順

- グッズタグはマッチング条件に使う。
- タグは1つ以上一致すれば `ユーザー×タグでマッチ` に出す。
- タグが複数一致する場合は、同じ棚・同じ条件ランク内で表示順位を上げる。
- タグ一致は `グッズ条件◎/○/△` の判定には入れない。

### Wish の L1 / L2 選択ルール

- L1 = グループ / 作品。
- L2 = メンバー / キャラクター。
- L2 マスターに値がある場合、Wish の L2 無指定は禁止する。
- L2 マスターに値がない場合だけ、L1 そのものを Wish 対象として選択できる。
### 構造化カラムと note

マッチングに使う値は note ではなく構造化カラムとして保存する。note は補足メモだけに使う。

| 項目 | 保存方針 |
|---|---|
| 求める L1 / L2 / グッズ種別 | 構造化カラム |
| 求めるタグ | 構造化カラム |
| 求める数量 | 構造化カラム |
| 条件種別（Wishから選ぶ / 条件から選ぶ / 定価） | 構造化カラム |
| 定価条件の金額 | 構造化カラム |
| 交換方法 | 構造化カラム |
| 現地交換の都道府県 | 構造化カラム |
| 支払い条件 | 構造化カラム |
...
ホーム、検索、打診作成、個別募集作成では、実在庫 `quantity` ではなく `market_available_qty` を使う。

```
market_available_qty = quantity - locked_qty
```

`locked_qty` は、個別募集・打診・取引で既に確保済みの数量を表す。`market_available_qty <= 0` のグッズは、ホーム、検索、打診候補から出さない。履歴保持のため物理削除せず、論理削除・非表示・closed/archived 相当で扱う。

Swift Native 実装では `goods_inventory.locked_qty` を保存し、`goods_inventory.market_available_qty` を生成列として持つ。打診が `agreed` へ遷移する直前に双方の譲在庫の残数を行ロック付きで検証し、成立後は proposal trigger で `locked_qty` を再計算する。取引完了時は proposal が `completed` へ変わった時点でロックが外れ、完了RPCで実在庫 `quantity` を減算する。

### 譲るものから見るの優先順

`譲るものから見る` では、自分の在庫ごとに「それを求めている相手」を出す。並び順は次を優先する。

1. 相手の個別募集に一致する件数・強さが多い。
2. 相手の Wish に一致する。
3. タグ一致数が多い。
4. 交換条件が `◎` に近い。
5. `market_available_qty > 0` の候補だけを出す。

### 発見画面から次画面へ引き継ぐ情報

ホームのデザイン案を正とし、導線の形はデザインに合わせる。ただし、ホームで判定した根拠は次画面で失わない。

| 入口 | 引き継ぐ情報 |
|---|---|
| ユーザー×タグでマッチ | 相手ユーザー、相手グッズ、自分の譲候補、一致タグ、グッズ条件、交換条件、支払い条件 |
| ユーザーでマッチ | 相手ユーザー、相手グッズ、自分の譲候補、グッズ条件、交換条件、支払い条件 |
| 譲るものから見る | 自分の譲グッズ、それを求める相手、個別募集合致、Wish一致、交換条件、支払い条件 |
| 個別募集に一致する候補 | 個別募集ID、相手の提示物、相手の求める条件、自分の一致グッズ |

---

## 0. 元メモ（原文）

> ホーム画面ではマッチング結果が表示されるようにする。
...
- データ構造としては、個別募集は **譲アイテム × wish のセット** + 比率
...
オーナーが提案している交換のあり方は、ベース登録と個別募集に整理できる。
...
### 4-1. wish 画面（hub-screens.jsx の wish タブ更新）
...
### 4-2. 在庫一覧 → 個別募集アクション（B-Inventory 拡張）
...
### 4-3. 打診画面（propose-select.jsx の C-0 拡張）
...
| 領域 | 既存 | v2 提案 | 影響度 |
|---|---|---|---|
| ホーム画面 | 4 タブ固定 | ホーム候補棚 + 個別募集 | **大** |
| wish 属性 | flexibility | + exchange_type | 小（DB 列追加） |
| 譲属性 | `kind` / `status` / 数量 | listing で拡張 | 中 |
| 個別募集 | 存在しない | 新テーブル `listings` | **大**（新規実装） |
| 打診 | テンプレ＋編集 | + calendar 公開フラグ | 中 |
| マッチング | 完全/片方向 2 軸 | 通常マッチ + listing | 中（ロジック追加） |
...
### Phase B — 個別募集（listings）
- migration: `listings`（**wish_id 必須**）
- 個別募集作成画面: 既存 wish 一覧から選んで「これで募集する」
- 譲アイテム選択 → wish 選択 → 比率 1:1〜10:1 → タグ
- マッチングロジック: listing 一致を通常マッチより前に表示
- UI 表記は **「個別募集」**で統一（"listing" は出さない）

### Phase C — 打診のカレンダー公開（重ね見 UI）
...
4. **カレンダー公開の範囲**: → 重ね見、取引終了で自動非公開（§A-5）
5. **「listing」用語**: → UI では「個別募集」で統一（§A-7）
...
2. **iter62 以降の実装計画（順序）**:
   ...
   - **iter64**: Phase B（個別募集 = listings）
   - **iter65**: Phase C（打診カレンダー公開 + overlay UI）
