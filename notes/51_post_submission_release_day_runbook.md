...
まず `notes/69_app_review_rejection_triage_runbook.md` で、Apple本文の保存、Guideline分類、同じbuildで直す/new buildにする/却下itemを削除する/appealする判断を記録する。

| 指摘 | 初動 |
|---|---|
| メタデータだけ | `notes/69` で同じbuild対応可否を判断し、`notes/41` のMetadataテンプレートで返信案を作る |
...
| コード修正が必要 | 開発セッションへ切り出し、`notes/65` で新ビルドを受領して再提出 |
