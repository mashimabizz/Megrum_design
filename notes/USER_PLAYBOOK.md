...
### 🎪 マーケティングフェーズ（2026-07-10〜・開発と並行）

マーケ関連の文書は3枚構成。**どのAIモデル（Claude各種・Codex等）でもこの3枚を読めば作業を再現できる**：

| ファイル | 役割 | 更新頻度 |
|---|---|---|
| `notes/78_marketing_plan.md` | 戦略・フェーズ計画・KPI・予算（読む） | 方針転換時のみ |
| `notes/79_marketing_action_tracker.md` | 実行トラッカー：週次タスク・状態・判断ログ（**毎週更新する**） | 高 |
| `notes/80_marketing_copy_kit.md` | 語彙ルール・法務チェック・文面テンプレ（コピペで使う） | 素材追加時 |

前提（78 §1 に詳細）：オーナーは**大阪在住・関西拠点**／**交換会の自主開催はしない**／公式Xは運用済み／郵送交換対応あり。

#### オーナーの指示テンプレ（コピペ用）

```
notes/79_marketing_action_tracker.md を見て、今週のタスクのうち
Claude担当のものを進めて。終わったらトラッカーのチェックと
実施ログを更新して。
```

```
notes/79 のタスク◯◯をやって。文面は notes/80 の語彙ルールと
法務チェックリストを通してから出して。
```

```
マーケの状況を教えて。notes/79 の進捗と、次に私（オーナー）が
やるべきことを一覧にして。
```

#### AIモデル側の約束

- 発信文面を作る時は必ず `notes/80` の語彙ルール・法務チェックを通す（「出会い」禁止等）
- タスクを実行・完了したら `notes/79` のステータスと実施ログを更新してから commit する
- 交換会・オフラインイベントの自主開催は提案しない（オーナー方針）
- 戦略を変える提案は `notes/78` の改訂として出し、オーナー承認を得てから 79 に展開する
