...
1. [現在のフェーズ](#1-現在のフェーズ)
2. [Codexとの付き合い方](#2-codexとの付き合い方)
3. [Phase 2 タスク一覧](#3-phase-2-タスク一覧)
4. [スマホからの指示テンプレ（コピペ用）](#4-スマホからの指示テンプレコピペ用)
5. [共通チェックポイント](#5-共通チェックポイント)
6. [スマホ vs PC の使い分け](#6-スマホ-vs-pc-の使い分け)
7. [スマホ指示のコツ](#7-スマホ指示のコツ)
8. [トラブルシューティング](#8-トラブルシューティング)
9. [次にやることの判断フロー](#9-次にやることの判断フロー)
10. [更新ルール](#10-更新ルール)
...
Codexには、速度と検証の重さを分けて依頼できる。

```
高速モードで：この表示だけ直して。検証は軽くてOK。
標準モードで：通常どおり、対象テストと画面確認までして。
リリース前モードで：広めにテストして、未確認項目も出して。
```

使い分け：
- **高速モード**: 休日に画面を見ながら、位置・余白・文字サイズ・文言・非表示などを連続で直す時。`notes/08` 記録やSimulator再ビルドは毎回必須にしない。
- **標準モード**: 機能、データ表示、ナビゲーション、保存処理、画面単位の修正。対象テストやビルド、必要ならスクショまで確認する。
- **リリース前モード**: TestFlight、App Store、法務、認証、通知、課金、本番DBに関係する作業。証跡を厚めに残す。

高速モードでも、保存処理・DB/API・認証・課金・通知・取引状態・打診/合意/完了に触る場合は、Codex側で標準モード以上に切り替える。

### 指示の基本形

Codexへ依頼する時は、できるだけ次の4点を渡す。
...
```
対象画面：ホーム検索のフィルター
期待する動作：グループ未選択時はメンバーとグッズタグを出さない
今起きている問題：グループを選ぶを押しても選択画面に進まない
確認してほしい端末 / ビルド：Megrum Preview iOS、最新OTA
```

### Codexに任せる時の約束

- 1回の依頼では、なるべく1つの障害モードまたは1つの画面改善に絞る。
- Codexには「再現 → 証拠 → 最小修正 → 再検証」を求める。
- 修正後は、実行したコマンド、Preview OTAのUpdate group、未確認のことを報告してもらう。
...
この言い方にすると、仕様変更とリファクタリングが混ざりにくい。

### バグ調査を頼む時

```
このバグを、まず再現手順を確定してから直してください。
可能ならログ・スクショ・該当データを見て原因を特定し、
最小修正後に同じ手順で再確認してください。
```
